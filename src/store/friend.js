...
const mutations = {
  ...
  removeFriendsInvitations({ friendInvitation }, newFriend) {
    friendInvitation.user.splice(
      friendInvitation.user.findIndex((v) => v._id === newFriend._id),
      1
    );
  },
  ...
  setUnFriend({ friends }, newFriend) {
    friends.user.splice(
      friends.user.findIndex((v) => v._id === newFriend._id),
      1
    );
  },

  setNotFriend({ notFriends }, newFriend) {
    notFriends.users.push(newFriend);
  },

  setAbortFriend({ requestFriend }, newFriend) {
    requestFriend.user.splice(
      requestFriend.user.findIndex((v) => v._id === newFriend._id),
      1
    );
  },

  setFriendsInvitations: (state, friendInvitation) =>
...
const actions = {
  ...
  async fetchFriendsInvitations({ commit }, userId) {
    const response = await axios.get(
      URL + "users/getfriendinvitation/" + userId
    );
  ...
  async requestFriend({ commit }, userData) {
    const response = await apiClient.post(
      URL + "users/requestfriend/" + userData._id,
      userData
    );
  ...
  async acceptFriend({ commit }, userData) {
    const response = await apiClient.post(
      URL + "users/acceptfriend/" + userData._id,
      userData
    );
  ...
  async unFriend({ commit }, userData) {
    const response = await apiClient.post(
      URL + "users/unfriend/" + userData._id,
      userData
    );
  ...
  async abortFriend({ commit }, userData) {
    const response = await apiClient.post(
      URL + "users/abortfriend/" + userData._id,
      userData
    );
