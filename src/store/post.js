...
const mutations = {
  ...
  addComment(state, comment) {
    const objIndex = state.posts.posts
      .map(function (x) {
        return x._id;
      })
      .indexOf(comment._id);
  ...
  deleteComment(state, id) {
    ...
    state.posts.posts[objIndex].comments = [
      ...state.posts.posts[objIndex].comments.filter(
        (item) => item._id !== id.commentId
      ),
    ];
  },
  ...
  updateCommentLike(state, like) {
    const objIndex = state.posts.posts
      ...
      .indexOf(like.postId);
    const comIndex = state.posts.posts[objIndex].comments
      .map(function (x) {
        return x._id;
      })
      .indexOf(like._id);
    Object.assign(state.posts.posts[objIndex].comments[comIndex], {
      likes: like.likes,
    });
  },
};
...
const actions = {
  ...
  async createComment({ commit }, commentData) {
    await apiClient
      .post(URL + "posts/comment", commentData)
      .then((response) => {
  ...
  async commentDelete({ commit }, postData) {
    await apiClient
      .put(URL + "posts/comment/" + postData[0].comment._id, postData)
      .then((response) => {
  ...
  async postUpdate({ commit }, postData) {
    await apiClient
      .put(URL + "posts/" + postData._id, postData)
      .then((response) => {
  ...
  async postDelete({ commit }, postData) {
    await apiClient
      .delete(URL + "posts/" + postData._id, postData)
      .then((response) => {
  ...
  async likeUpdate({ commit }, likeData) {
    await apiClient
      .post(URL + "posts/like/" + likeData.id, likeData)
      .then((response) => {
  ...
  async likeCommentUpdate({ commit }, likeData) {
    await apiClient
      .post(URL + "posts/comment/like/" + likeData.id, likeData)
      .then((response) => {
