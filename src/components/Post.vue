<template>
  <div class>
    <transition name="slide-fade" v-if="!enterPost" mode="out-in">
      <div class>
        <button type="submit" class="btn-sub post-new" @click="newPost()" value>
          {{ btnName }}
        </button>
      </div>
    </transition>
    <transition name="slide-fade" v-else mode="out-in">
      <form @submit.prevent="addPost()" method="post" class="log">
        ...
        <label
          class="post__label"
          for="image"
          :style="{ visibility: showLabelImage }"
          >Image</label
        >
        <input
          @focus="clearErrors"
        ...
        <label
          class="post__label"
          for="desc"
          :style="{ visibility: showLabelDesc }"
          >Content</label
        >
        <ckeditor
          :editor="editor"
        ...
        <div class="post-footer">
          <button @click.prevent="newPost()" class="btn-sub btn-cancel" value>
            Cancel
          </button>
          <button type="submit" class="btn-sub btn-save" value="Log In">
            Save
          </button>
        </div>
